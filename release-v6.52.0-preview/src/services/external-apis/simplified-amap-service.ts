...
export class SimplifiedAmapService {
  ...
  /**
   * MCP工具调用方法（通过 MapFailoverService 间接调用不同提供商的 MCP 工具）
   */
  private async callMcpTextSearch(params: Record<string, string>): Promise<any> {
    try {
      const { MapFailoverService } = await import('@/services/failover/map-failover-service');
      const mapFailover = new MapFailoverService();
      const r = await mapFailover.query({ type: 'poi_search', params: {
        keywords: params.keywords || '餐厅',
        city: params.city || '北京',
        types: params.types || '050000'
      }});
      return r.result || { status: '1', info: 'OK', pois: [] };
    } catch (error) {
  ...
  private async callMcpGeocoding(params: Record<string, string>): Promise<any> {
    try {
      const { MapFailoverService } = await import('@/services/failover/map-failover-service');
      const mapFailover = new MapFailoverService();
      const r = await mapFailover.query({ type: 'geocode', params: {
        address: params.address,
        city: params.city
      }});
      return r.result || { status: '1', info: 'OK', geocodes: [] };
    } catch (error) {
  ...
  private async callMcpWeather(params: Record<string, string>): Promise<any> {
    try {
      const { MapFailoverService } = await import('@/services/failover/map-failover-service');
      const mapFailover = new MapFailoverService();
      const r = await mapFailover.query({ type: 'weather', params: { city: params.city }});
      return r.result || { status: '1', info: 'OK', lives: [] };
    } catch (error) {
  ...
  private async callMcpAroundSearch(params: Record<string, string>): Promise<any> {
    try {
      const { MapFailoverService } = await import('@/services/failover/map-failover-service');
      const mapFailover = new MapFailoverService();
      const r = await mapFailover.query({ type: 'around', params: {
