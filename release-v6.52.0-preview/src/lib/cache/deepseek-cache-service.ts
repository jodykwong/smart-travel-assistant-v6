/**
 * 智游助手v6.1 - LLM缓存服务（内置双链路冗余）
 * 在原有 DeepSeek 缓存服务基础上，接入 LLMFailoverService 实现主备切换
 */

import { getCacheService } from './cache-service';
import crypto from 'crypto';
import { LLMFailoverService, ChatRequest } from '@/services/failover/llm-failover-service';
...
class DeepSeekCacheService {
  ...
  private async callDeepSeekAPI(request: DeepSeekRequest): Promise<DeepSeekResponse> {
    // 通过 LLMFailoverService 统一调用，保留原返回结构
    const { result } = await this.llm.chat(request as unknown as ChatRequest);
    return {
      ...result,
      cached: false,
