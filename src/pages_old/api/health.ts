...
async function healthHandler(
  ...
  try {
    ...
    // 更新监控指标
    updateMetrics({
      activeUsers: Math.floor(Math.random() * 100) + 50, // 模拟活跃用户数
      cacheHitRate: memoryPercentage < 50 ? 0.9 : 0.7, // 基于内存使用率模拟缓存命中率
      databaseConnections: 1, // 当前连接数
    });
...
// 导出带监控的处理器
export default withMetrics(healthHandler, {
  service: 'smart-travel-v6.2',
  excludePaths: [] // 不排除健康检查，我们想监控它
});
