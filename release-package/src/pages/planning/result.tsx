...
const parseTimelineActivities = (dayContent: string, destination: string) => {
  ...
  // 修复：支持多种时间段格式，包括Markdown格式
  const timeBlockRegex = /(?:^|\n)\s*(?:-\s*)?\*\*\s*(上午|下午|晚上|早上|中午)\s*\*\*\s*/gm;
  const timeBlocks = [];
  let lastIndex = 0;
  let match;
  ...
  if (activities.length === 0) {
    ...
    // 如果仍然没有找到，尝试解析活动列表格式（如第3天的格式）
    if (activities.length === 0) {
      console.log('🔍 尝试解析活动列表格式...');
      const activityListPattern = /\*\*活动\*\*\s*\n([\s\S]*?)(?=\n\*\*|$)/;
      const activityMatch = dayContent.match(activityListPattern);

      if (activityMatch) {
        const activityContent = activityMatch[1];
        const activityItems = activityContent.split(/\n-\s*/).filter(item => item.trim().length > 10);
        ...
        activityItems.forEach((item, index) => {
          ...
          if (cleanItem.length > 10) {
            ...
            if (cleanItem.includes('下午') || cleanItem.includes('午后')) {
              period = '下午';
              time = '14:00-17:00';
            } else if (cleanItem.includes('上午') || cleanItem.includes('早上')) {
              period = '上午';
              time = '09:00-12:00';
            } else if (index === 0) {
              period = '上午';
            ...
            activities.push({
              ...
              period: getPeriodFromTime(period),
              title: extractActivityTitle(cleanItem),
              description: enhanceActivityDescription(cleanItem, period),
              icon: getActivityIcon(cleanItem),
              cost: extractCostFromDescription(cleanItem) || generateReasonableCost(cleanItem),
              duration: extractDurationFromDescription(cleanItem) || '约2-3小时',
              color: getActivityColor(period)
            });
          }
        });
    ...
    // 如果还是没有找到，尝试解析数字列表格式（如长春的格式）
    if (activities.length === 0) {
      console.log('🔍 尝试解析数字列表格式...');
      const numberedListPattern = /(\d+)\.\s*\*\*([^*]+)\*\*[：:]\s*([^\\n]*(?:\\n(?!\d+\.)[^\\n]*)*)/g;
      let numberedMatch;

      while ((numberedMatch = numberedListPattern.exec(dayContent)) !== null) {
        const number = numberedMatch[1];
        const timeLabel = numberedMatch[2].trim();
        ...
        if (description && description.length > 10) {
          ...
          if (timeLabel.includes('早餐') || timeLabel.includes('上午')) {
            period = '上午';
            time = '09:00-12:00';
          } else if (timeLabel.includes('午餐') || timeLabel.includes('中午')) {
            period = '中午';
            time = '12:00-14:00';
          } else if (timeLabel.includes('下午')) {
            period = '下午';
            time = '14:00-17:00';
          } else if (timeLabel.includes('晚餐') || timeLabel.includes('傍晚') || timeLabel.includes('晚上')) {
            period = '晚上';
            time = '18:00-21:00';
          } else {
            // 根据序号推断时间
          ...
          activities.push({
            time: normalizeTimeString(time),
            period: getPeriodFromTime(period),
            title: extractActivityTitle(timeLabel + ': ' + description),
            description: enhanceActivityDescription(description, period),
            icon: getActivityIcon(description),
            cost: extractCostFromDescription(description) || generateReasonableCost(description),
            duration: extractDurationFromDescription(description) || '约2-3小时',
...
// 从内容中提取真实的日标题
const extractDayTitle = (content: string, day: number): string => {
  // 尝试匹配 "Day X：标题" 格式
  const dayTitleMatch = content.match(/Day\s*\d+[：:]\s*([^*\n]+)/i);
  if (dayTitleMatch) {
    return dayTitleMatch[1].trim();
  }
    ...
    try {
      ...
      if (result.success && result.data) {
        let planData = {
          ...
          legacyFormat: result.data.legacyFormat,
          parseSuccess: result.data.parseSuccess,
          timelineVersion: result.data.timelineVersion,
        };

        // 优先使用API提供的标准行程（API优先 + 为失败而设计）
        let apiItinerary: Array<{ day: number; title: string; content: string }> = [];
        ...
        if (result.data.result) {
          try {
            ...
            if (Array.isArray(sessionResult.itinerary) && sessionResult.itinerary.length > 0) {
              apiItinerary = sessionResult.itinerary;
            }

            // Timeline解析架构v2.0：获取服务端解析的legacyFormat数据
            if (sessionResult.legacyFormat) {
              planData.legacyFormat = sessionResult.legacyFormat;
              planData.parseSuccess = sessionResult.parseSuccess;
              planData.timelineVersion = sessionResult.timelineVersion;
              console.log('[DATA] 从API获取Timeline解析架构v2.0数据', {
                legacyFormatLength: sessionResult.legacyFormat.length,
                parseSuccess: sessionResult.parseSuccess,
                timelineVersion: sessionResult.timelineVersion
              });
            }
          } catch (parseError) {
        ...
        // Timeline解析架构v2.0：优先使用服务端解析的legacyFormat
        let activities = [] as DayActivity[];

        // 检查是否有Timeline解析架构v2.0的结果
        if (planData.legacyFormat && Array.isArray(planData.legacyFormat) && planData.legacyFormat.length > 0) {
          console.log('[DATA] 使用Timeline解析架构v2.0结果, length=', planData.legacyFormat.length);
          console.log('[DATA] parseSuccess=', planData.parseSuccess, 'timelineVersion=', planData.timelineVersion);

          // 直接使用服务端解析好的legacyFormat数据
          activities = planData.legacyFormat.map((legacyDay: any) => ({
            day: legacyDay.day,
            title: legacyDay.title,
            date: legacyDay.date,
            weather: legacyDay.weather,
            temperature: legacyDay.temperature,
            location: legacyDay.location,
            cost: legacyDay.cost,
            progress: legacyDay.progress,
            image: legacyDay.image || getSmartDefaultImage(planData.destination),
            tags: legacyDay.tags || [],
            timeline: legacyDay.timeline || [], // 已经是完整的timeline格式
          }));

          console.log('[DATA] Timeline解析架构v2.0数据处理完成', {
            daysCount: activities.length,
            totalActivities: activities.reduce((sum, day) => sum + day.timeline.length, 0)
          });

        } else if (apiItinerary.length > 0) {
          console.log('[DATA] 回退到旧版解析, usingApiItinerary=true, length=', apiItinerary.length);
          activities = apiItinerary.map((d) => {
            // 计算每天的正确日期
            const dayDate = new Date(planData.startDate || Date.now());
            dayDate.setDate(dayDate.getDate() + (d.day - 1));

            // 提取真实的日标题
            const realTitle = extractDayTitle(d.content, d.day);

            return {
              day: d.day,
              title: realTitle,
              date: dayDate.toLocaleDateString('zh-CN', { month: 'long', day: 'numeric', weekday: 'short' }),
              weather: generateReasonableWeather(d.day, planData.destination),
              temperature: generateReasonableTemperature(d.day, planData.destination),
              location: planData.destination,
              cost: 0,
              progress: Math.floor(Math.random() * 30) + 70,
              image: getSmartDefaultImage(planData.destination),
              tags: generateIntelligentTags(realTitle, []),
              timeline: parseTimelineActivities(d.content, planData.destination),
            };
          });
        } else {
          console.log('[DATA] 最终回退到本地解析');
          activities = parseDayActivities(
